...
# using inversion/generation extract intermediate feature maps from the model's latent representations. 
# control the diffusion model's operation
# handle conditioning with prompts
# manage the generation or inversion pipeline while capturing intermediate features for analysis or visualization

################## def_init_ ####################
# Initialize scheduler, random generator, batch size, etc.
# Initialize UNet, VAE, CLIP models and set prompts
# Set up diffusion mode, indices, output resolution

################  Methods #########################
# change_cond: Update-----> conditioning prompt --------> positive(conditional)/negative(conditional) prompts
# run_generation: forward_diffusion_process (latent code -----> generated image (step by step))
# run_inversion: reversed_diffusion_process (image --------> latent code)
# get_feats: extracts feature(intermediate) ----------> feature(diffusion)
# latents_to_images: latent -----(using vae)------> image
#### forward: 
...
class DiffusionExtractor:
    ...
    def run_generation(self, latent, guidance_scale=-1, min_i=None, max_i=None):  ##### Runs the generation process from latent to generated output
    ...
    def run_inversion(self, latent, guidance_scale=-1, min_i=None, max_i=None):    ###  Runs the inversion process to reconstruct latent from image
        xs = generalized_steps(
            latent, 
            self.unet, 
            self.scheduler, 
    ...
    def get_feats(self, latents, extractor_fn, preview_mode=False):
        ...
        if not preview_mode:
            init_resnet_func(self.unet, save_hidden=True, reset=True, idxs=self.idxs, save_timestep=self.save_timestep) #init_resnet_func initializes the UNet to store hidden states for specific timesteps (self.save_timestep) during the diffusion process.
        outputs = extractor_fn(latents)
        if not preview_mode:
            feats = []
            for timestep in self.save_timestep:
                timestep_feats = collect_and_resize_feats(self.unet, self.idxs, timestep, self.output_resolution) ##After each diffusion step, collect_and_resize_feats retrieves features and resizes them to self.output_resolution.
    ...
    def forward(self, images=None, latents=None, guidance_scale=-1, preview_mode=False):
        if images is None:  #If images are not provided, it initializes random latent codes and starts a generation or inversion process based on self.diffusion_mode.
            if latents is None:
                latents = torch.randn((self.batch_size, self.unet.in_channels, 512 // 8, 512 // 8), device=self.device, generator=self.generator) ## kno latent nai, banao new latent. 
